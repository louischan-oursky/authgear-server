...
func (s *Service2) doPost(
	...
	for {
		...
		graph, edges, err = s.runGraph(session.ID, result, inputFn, graphFn)
...
func (s *Service2) runGraph(
	...
	var graph *interaction.Graph
	var edges []interaction.Edge
	interactionErr := s.Graph.DryRun(sessionID, func(ctx *interaction.Context) (*interaction.Graph, error) {
		var err error
