...
type AuthflowControllerAuthflowHTTPClient interface {
	...
	Input(w http.ResponseWriter, stateToken string, input map[string]interface{}) (*authflowclient.FlowResponse, error)
}
...
func (c *AuthflowController) HandleResumeOfFlow(
	...
	emptyStateToken := ""
	flowResponse, err := c.Authflows.Input(w, emptyStateToken, input)
	if err != nil {
...
func (c *AuthflowController) createAuthflow(r *http.Request, s *webapp.Session, flowReference authflowclient.FlowReference) (*authflowclient.FlowResponse, error) {
	// FIXME(authflow): construct query
	query := ""
	return c.Authflows.Create(flowReference, query)
}
...
func (c *AuthflowController) ReplaceScreen(w http.ResponseWriter, r *http.Request, s *webapp.Session, flowReference authflowclient.FlowReference, input map[string]interface{}) (result *webapp.Result, err error) {
	...
	screen, err = c.takeBranchRecursively(w, s, screen)
	if err != nil {
		return
	}
...
func (c *AuthflowController) AdvanceWithInput(w http.ResponseWriter, r *http.Request, s *webapp.Session, screen0 *webapp.AuthflowScreenWithFlowResponse, input map[string]interface{}) (result *webapp.Result, err error) {
	...
	flowResponse1, err := c.Authflows.Input(w, screen0.Screen.StateToken.StateToken, input)
	if err != nil {
		return
	}
	...
	screen2, err := c.takeBranchRecursively(w, s, screen1)
	if err != nil {
		return
	}
...
func (c *AuthflowController) UpdateWithInput(w http.ResponseWriter, r *http.Request, s *webapp.Session, screen *webapp.AuthflowScreenWithFlowResponse, input map[string]interface{}) (result *webapp.Result, err error) {
	...
	newF, err := c.Authflows.Input(w, screen.Screen.StateToken.StateToken, input)
	if err != nil {
		return
	}
...
func (c *AuthflowController) takeBranchRecursively(w http.ResponseWriter, s *webapp.Session, screen *webapp.AuthflowScreenWithFlowResponse) (newScreen *webapp.AuthflowScreenWithFlowResponse, err error) {
	for screen.HasBranchToTake() {
		...
		case webapp.TakeBranchResultInput:
			...
			flowResponse, err = c.Authflows.Input(w, screen.Screen.StateToken.StateToken, takeBranchResult.Input)
			if err != nil {
				return
			}
