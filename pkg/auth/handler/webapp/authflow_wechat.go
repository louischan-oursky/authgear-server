...
type AuthflowWechatHandlerOAuthStateStore interface {
	GenerateState(state *webappoauth.WebappOAuthState) (stateString string, err error)
}
...
func (h *AuthflowWechatHandler) GetData(w http.ResponseWriter, r *http.Request, s *webapp.Session, screen *webapp.AuthflowScreenWithFlowResponse) (map[string]interface{}, error) {
	...
	stateString, err := h.OAuthStateStore.GenerateState(state)
	if err != nil {
		return nil, err
	}
	...
	authorizationURL = urlutil.WithQueryParamsAdded(authorizationURL, map[string]string{"state": stateString})
