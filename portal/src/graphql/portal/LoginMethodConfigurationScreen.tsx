...
function MatrixNoChoice() {
  const theme = useTheme();
  return (
    <div className={styles.maxtrixNoChoice}>
      <Text
        block={true}
        variant="medium"
        styles={{
      ...
      >
        <FormattedMessage id="LoginMethodConfigurationScreen.matrix.no-login-needed" />
      </Text>
    </div>
  );
}
...
function Matrix(props: MatrixProps) {
  ...
  const customChecked = loginMethod === "custom";
